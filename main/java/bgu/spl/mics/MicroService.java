...
public abstract class MicroService implements Runnable {
    private static final Logger logger = LogManager.getLogger(MicroService.class.getSimpleName());
    private static MessageBus messageBus = MessageBusImpl.getInstance();
    private ConcurrentHashMap<Class<? extends Message>, Callback<?>> messagesCallbacks;
    ...
    public MicroService(String name) {
        this.name = name;
        messagesCallbacks = new ConcurrentHashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        messagesCallbacks.putIfAbsent(type, callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        logger.info(this.getName() + " is sending a message event: " + e.getClass().getSimpleName());
    ...
    protected final <T> void complete(Event<T> e, T result) {
        logger.info(this.getName() + " is resolving a message: " + e.getClass().getSimpleName() + ", Result: " + result);
        ...
        if(isActiveListener()) {
            while (!terminated) {
                try {
                    ...
                    callbackHelper(messagesCallbacks.get(message.getClass()), message);
    ...
    private <T> void callbackHelper(Callback<?> callback, T c) {
        ((Callback<T>) callback).call(c);
    }

    private boolean isActiveListener(){
        return !messagesCallbacks.isEmpty();
    }
